# Function App: FA-ATTACH-SUMMARY
# Created by: Joshua Wilshere
# Created on: 4/23/25
# Purpose: Pass an incoming PDF, Word, TXT, or Outlook file (.msg) email attachment file from a blob storage account to the Azure AI Document
#           Intelligence service and write the text output back to a record in Cosmos DB
#          Attachments are landed in the raw/email_attachment blob storage location by the main function app when it processes a .msg file, or when this
#           function app processes an attached .msg file and saves the attachments of the attached email message to this blob storage location
# 
...
#   func azure functionapp publish "FA-attach-summary"
...
@app.blob_trigger(arg_name="blobtriggerfile",
                  #source="EventGrid", 
                  path="raw/email_attachments/{subPath}/{name}",
...
def func_app_email_attachment_summary_main(blobtriggerfile: func.InputStream, cosmodocsout: func.Out[func.Document], context: func.Context):
    ...
    source_email = f"{os.path.split(os.path.dirname(blobtriggerfile.name))[1]}.msg"
    ...
    if "/Archive" in dirname:
        ...
        return
    if file_extension.lower() in allowable_extensions:
        logging.info(f"{filename} is a PDF or Word Doc, beginning file processing")
        func_app_email_attachment_summary(blobtriggerfile, cosmodocsout, context, filename, dirname, file_extension, source_email)
    else:
        logging.info(f"{filename} is not a PDF or a Word Document, skipping file processing")
        return


# Primary function
def func_app_email_attachment_summary(blobtriggerfile: func.InputStream, cosmodocsout: func.Out[func.Document], context: func.Context, filename, dirname, file_extension, source_email):
    ...
    try:
        ...
        # Initialize Cosmos DB connection and client
        cosmosdb_conn = os.environ.get("cosmosdb_CONNECTION")
        cosmos_client = CosmosClient.from_connection_string(cosmosdb_conn)
        cosmos_db = cosmos_client.get_database_client(AZURE_COSMOS_DATABASE_NAME)
        cosmos_container = cosmos_db.get_container_client(AZURE_COSMOS_CONTAINER_NAME)
        
        # Query Cosmos DB for existing records for the incoming blob name
        # Note - query is on full blob name, not just the file name, as attachments may not have unique file names
        # This does carry the risk of processing the same file multiple times if the same exact file was attached to multiple emails
        cosmos_query = f"SELECT * FROM c WHERE c.blobname = '{blobtriggerfile.name}'"
        cosmosdocsin = list(cosmos_container.query_items(query=cosmos_query, enable_cross_partition_query=True))
        
        record_count = len(cosmosdocsin)
    ...
    ##################################################
        ...
        #         if len(filemarkings) <= 0 and len(pypdf2_text_dict['pypdf2_fulltext']) > 0:
        #             filemarkings = extract_classification(pypdf2_text_dict['pypdf2_fulltext'], classifications)
    ...
    #########################
        ...
        if summary_missing_flag and txt_result_status == 'succeeded' and len(txt_result.strip()) > 0:
            ...
            # Submit each chunk to the summarization service and then append the results together
                ...
                if len(final_summary.strip()) > 0:
                    ...
                    logging.info(f"Document complete at: {summary_finish_timestamp}")
        ...
        # Else, if the text extraction does not succeed, check to see if any extracted text is returned, and then write an error in the record and to the log
            ...
            if 'error' not in dict_output:
                dict_output['error'] = {}
            if text_missing_flag and len(txt_result.strip()) > 0 and len(dict_output['fulltextextract']) == 0:
                ...
                if text_missing_flag and (len(txt_result.strip()) == 0 or len(dict_output["fulltextextract"]) == 0):
                    dict_output["fulltextextract"] = 'No extractable text found'
                    dict_output['error']['text_extraction_status'] = txt_result_status
        ...
        if msg.attachments:
            ...
            for attachment in msg.attachments:
                if attachment.longFilename:
                    ...
                    attachment_name = f"email_attachments/attachments_of_attachments/{base_filename}/{attachment.displayName}"

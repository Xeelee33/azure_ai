...
resource "azurerm_linux_function_app" "linux_function_app" {
  app_settings = {
    ...
    # This settings is managed via the storage_account_* settings further down and should not be managed manually
    # Reference documentation: https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/linux_function_app#:~:text=in%20app%20setting.-,Note%3A,-For%20storage%20related
    # AzureWebJobsStorage          = "@Microsoft.KeyVault(SecretUri=${data.azurerm_key_vault.keyvault.vault_uri}secrets/${azurerm_key_vault_secret.kv_function_app_storage_conn.name})"
  ...
  ## In post deployment, uncomment out this attribute and comment out storage_account_name and storage_account_access_key above
...
resource "azurerm_monitor_private_link_scoped_service" "ampls_app_insights_link" {
  name                = "ampls-link-${azurerm_application_insights.apin_function_app.name}"
